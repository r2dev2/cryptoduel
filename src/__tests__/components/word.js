...
const getDecrypted = el => el.parentElement.querySelector('.decrypted-letter');
...
describe('<Word />', () => {
  ...
  it('shows all replaced letters', () => {
    const { getByText } = render(Word, {
      word: testWord, replacement: testReplacement
    });
  ...
  it('stops firing replace events when disabled', () => {
    const { getByText, component } = render(Word, { word: testWord, disabled: true });
    const cb = jest.fn();
